...
func (g *agentgatewayParametersHelmValuesGenerator) GetValues(ctx context.Context, obj client.Object) (map[string]any, error) {
	...
	applyManagedOIDCCookieSecretDefaults(vals.Agentgateway, gw.Name, g.oidcCollection.resolvedParametersRequireOIDC(resolved))
...
func (g *agentgatewayParametersHelmValuesGenerator) gatewayRequiresOIDCCookieSecret(resolved *resolvedParameters) bool {
	if g == nil || g.oidcCollection == nil {
		return false
	}
	return g.oidcCollection.resolvedParametersRequireOIDC(resolved)
}
