...
func newFetcher(cache *providerCache) *fetcher {
	return &fetcher{
		cache:                 cache,
		defaultProviderClient: &providerHTTPClientImpl{Client: remotefetch.MakeClient(nil)},
...
func (f *fetcher) fetchProviderConfig(ctx context.Context, source ProviderSource) (ProviderConfig, error) {
	...
	if source.TLSConfig != nil {
		provider, err = (&providerHTTPClientImpl{Client: remotefetch.MakeClient(source.TLSConfig)}).FetchProviderConfig(ctx, source.Target)
	} else {
