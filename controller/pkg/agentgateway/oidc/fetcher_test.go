...
func TestIssuerFromDiscoveryURL(t *testing.T) {
	tests := []struct {
		discoveryURL   string
		...
		{
			discoveryURL:   "https://issuer.example/.well-known/openid-configuration/tenant",
			expectedIssuer: "https://issuer.example/tenant",
		},
	}

	for _, tc := range tests {
		t.Run(tc.discoveryURL, func(t *testing.T) {
			assert.Equal(t, tc.expectedIssuer, issuerFromDiscoveryURL(tc.discoveryURL))
		})
	}
}
