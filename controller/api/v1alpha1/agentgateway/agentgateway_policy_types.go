...
type Traffic struct {
	...
	// `oidc` enables browser-based user login at the gateway via the OpenID
	// Connect Authorization Code Flow with PKCE. The gateway acts as a Relying
	// Party: it terminates the redirect flow, validates the ID token, and
	// issues an encrypted session cookie so upstream services see authenticated
	// requests. For bearer-token validation on machine-to-machine APIs, use
...
// OIDC configures the gateway as an OpenID Connect Relying Party for browser-
