...
// Macro to reduce serde boilerplate for Messages API types
macro_rules! messages_struct {
...
pub mod types {
    ...
    /// Request redacted thinking block (input)
    #[derive(Debug, Clone, Serialize, Deserialize)]
...
pub mod ingress {
    ...
    /// Lightweight validation for Messages API requests - fail fast on obvious errors only
    /// Lets LLM providers handle their own provider-specific validation rules
    pub fn validate_messages_request(m: &types::MessagesRequest) -> Result<(), AIError> {
        ...
        // 2. max_tokens must be > 0
        if m.max_tokens == 0 {
            return Err(AIError::InvalidMaxTokens(m.max_tokens));
        }

        // 3. Basic content validation - at least one message with non-empty content
        let has_content = m.messages.iter().any(|msg| match &msg.content {
            types::MessageContent::String(text) => !text.trim().is_empty(),
            types::MessageContent::Blocks(blocks) => !blocks.is_empty(),
        });
        ...
        match provider {
            "anthropic" => {
                if let Some(anthropic_data) = extract_anthropic_headers(headers) {
                    ...
                    if let Some(anthropic_value) = anthropic_data.get("anthropic") {
                        provider_data.insert("anthropic".to_string(), anthropic_value.clone());
                    } else {
                        // Log warning if expected structure is missing
                        tracing::warn!("Anthropic headers extracted but missing 'anthropic' key");
                    }
                }
            },
            _ => {
    ...
    pub(crate) fn has_system_cache_control(system: &types::SystemPrompt) -> bool {
        match system {
            types::SystemPrompt::String(_) => false, // String form doesn't support cache_control
            types::SystemPrompt::Blocks(blocks) => {
                blocks.iter().any(|block| block.cache_control.is_some())
            }
        }
    }
        ...
        match &msg.content {
            ...
            types::MessageContent::Blocks(blocks) => {
                ...
                let mut result = ContentBlockConversionResult {
                    ...
                    tool_results_order: Vec::new(),
                    tool_results_meta: HashMap::new(),
                    provider_data: ProviderData::default(),
                };

                // Initialize container for this message's blocks
                result.provider_data.anthropic_content_blocks_by_msg.push(Vec::new());
                let mut cache_markers = Vec::new();

                // Process each content block
                for block in blocks {
                    process_content_block(block, &mut result, beta_features, &mut cache_markers)?;
                }

                // Store cache markers for this message
                result.provider_data.cache_markers.per_message.push(cache_markers);
                ...
                // Create primary user/assistant message only if there's meaningful content
                let should_create_primary = match msg.role {
                    types::MessageRole::User => !result.text_parts.is_empty(),
                    types::MessageRole::Assistant => !result.text_parts.is_empty() || !result.tool_calls.is_empty(),
                };

                if should_create_primary {
                    let primary_msg = match msg.role {
                        types::MessageRole::User => {
                            ...
                            let combined_text = result.text_parts.join("\n\n");
                ...
                // Create separate Tool messages for each tool_result in original order
                // These will be coalesced by the Bedrock adapter to maintain role alternation
                for tool_use_id in result.tool_results_order {
                    if let Some(content) = result.tool_results.get(&tool_use_id) {
                        let tool_msg = universal::RequestMessage::Tool(
                            universal::RequestToolMessage {
                                tool_call_id: tool_use_id,
                                content: universal::RequestToolMessageContent::Text(content.clone()),
                            }
                        );
    ...
    struct ContentBlockConversionResult {
        ...
        /// Tool results order to preserve storytelling sequence
        tool_results_order: Vec<String>,
    ...
    /// Cache marker preservation for Bedrock cachePoint emission
    #[derive(Debug, Default, Serialize, Deserialize)]
    pub(crate) struct CacheMarkers {
        /// True if system prompt has cache_control
        pub system: bool,
        /// True if any tool has cache_control
        pub tools: bool,
        /// For each input message, an array of booleans aligned with the original block order
        /// true = cache this block
        pub per_message: Vec<Vec<bool>>,
    }

    /// Provider-specific data preservation for egress
    #[derive(Debug, Default)]
    pub(crate) struct ProviderData {
        /// Original Anthropic content blocks for lossless reconstruction (per message)
        anthropic_content_blocks_by_msg: Vec<Vec<types::RequestContentBlock>>,
        /// Cache markers for Bedrock cachePoint placement
    ...
    fn process_content_block(
        ...
        beta_features: &[String],
        cache_markers: &mut Vec<bool>, // Track cache_control per block
    ) -> Result<(), AIError> {
        // Always store the original block for lossless reconstruction
        result.provider_data.anthropic_content_blocks_by_msg.last_mut()
            .expect("message blocks container should exist")
            .push(block.clone());

        // Track if this block has cache_control
        let has_cache_control = match block {
            types::RequestContentBlock::Text(text_block) => text_block.cache_control.is_some(),
            types::RequestContentBlock::Image(image_block) => image_block.cache_control.is_some(),
            types::RequestContentBlock::Document(doc_block) => doc_block.cache_control.is_some(),
            types::RequestContentBlock::ToolUse(tool_use_block) => tool_use_block.cache_control.is_some(),
            types::RequestContentBlock::ToolResult(tool_result_block) => tool_result_block.cache_control.is_some(),
            types::RequestContentBlock::SearchResult(search_block) => search_block.cache_control.is_some(),
            types::RequestContentBlock::Thinking(_) => false,
            types::RequestContentBlock::RedactedThinking(_) => false,
        };
        cache_markers.push(has_cache_control);
        ...
        match block {
            ...
            types::RequestContentBlock::ToolUse(tool_use) => {
                // Validate that input is a JSON object as required by Anthropic
            ...
            types::RequestContentBlock::ToolResult(tool_result) => {
                ...
                // Store for separate Tool message creation (preserve order)
                result.tool_results.insert(tool_result.tool_use_id.clone(), final_content);
                result.tool_results_order.push(tool_result.tool_use_id.clone());
            ...
            // Thinking blocks: preserve in provider_data, don't add to text_parts
            // These will be re-emitted in Bedrock requests for tool follow-ups
            types::RequestContentBlock::Thinking(_thinking) => {
                ...
                // Thinking blocks are already preserved in provider_data.anthropic_content_blocks_by_msg
                // Don't add to text_parts - these are metadata for thinking mode
            },

            // Redacted thinking blocks: preserve in provider_data
            types::RequestContentBlock::RedactedThinking(_redacted) => {
                if !beta_features.iter().any(|f| f.contains("thinking")) {
                    tracing::warn!("Redacted thinking block present but no thinking-related anthropic-beta header found");
                }
                // Redacted thinking blocks are already preserved in provider_data.anthropic_content_blocks_by_msg
                // Don't add to text_parts - these are metadata for thinking mode
            },
        }
    ...
    /// Returns (tool_choice, parallel_tool_calls_enabled)
    fn convert_tool_choice(choice: &types::ToolChoice) -> (universal::ToolChoiceOption, Option<bool>) {
        let (choice_option, disable_parallel) = match choice {
            types::ToolChoice::Auto { disable_parallel_tool_use } => {
                (universal::ToolChoiceOption::Auto, *disable_parallel_tool_use)
            },
            types::ToolChoice::Any { disable_parallel_tool_use } => {
                (universal::ToolChoiceOption::Required, *disable_parallel_tool_use)
            },
            types::ToolChoice::Tool { name, disable_parallel_tool_use } => {
                let choice = universal::ToolChoiceOption::Named(universal::NamedToolChoice {
                    r#type: universal::ToolType::Function,
            ...
            types::ToolChoice::None => (universal::ToolChoiceOption::None, None),
        };

        let parallel_enabled = disable_parallel.map(|disabled| !disabled);
        (choice_option, parallel_enabled)
    }
    ...
    pub(crate) fn build_universal_messages(
        ...
        // 1. Add system message first (if present) and detect cache markers
        if let Some(system) = &messages_req.system {
            // Check if system has cache_control markers
            collected_provider_data.cache_markers.system = has_system_cache_control(system);
        ...
        // Check if any tools have cache_control
        if let Some(tools) = &messages_req.tools {
            collected_provider_data.cache_markers.tools = tools.iter().any(|tool| tool.cache_control.is_some());
        }
        ...
        for msg in &messages_req.messages {
            ...
            collected_provider_data.anthropic_content_blocks_by_msg.extend(msg_provider_data.anthropic_content_blocks_by_msg);
            collected_provider_data.cache_markers.per_message.extend(msg_provider_data.cache_markers.per_message);
    ...
    pub fn to_universal(
        ...
        if !content_provider_data.anthropic_content_blocks_by_msg.is_empty() {
            anthropic_data["content_blocks_by_msg"] = serde_json::to_value(&content_provider_data.anthropic_content_blocks_by_msg)
                .unwrap_or_else(|_| serde_json::json!([]));
        }

        // Store cache markers for Bedrock cachePoint emission
        anthropic_data["cache_markers"] = serde_json::to_value(&content_provider_data.cache_markers)
            .unwrap_or_else(|_| serde_json::json!({}));
        ...
        // Convert legitimate Anthropic thinking config to Universal reasoning
        let reasoning = m.thinking.as_ref().map(|t| {
            debug!("Messages: Converting thinking config - type: {:?}, budget_tokens: {:?}", t.thinking_type, t.budget_tokens);
            let reasoning = universal::Reasoning {
                enabled: matches!(t.thinking_type, types::ThinkingType::Enabled),
                effort: None, // Anthropic uses budget_tokens, not effort levels
                max_tokens: t.budget_tokens,
                exclude: false, // Anthropic thinking should be visible
            };
            debug!("Messages: Converted to reasoning - enabled: {}, max_tokens: {:?}", reasoning.enabled, reasoning.max_tokens);
            reasoning
        });

        // Build semantic cache plan ONLY if client explicitly requests cache control
        let cache = if m.system.as_ref().map(has_system_cache_control).unwrap_or(false)
            || m.tools.as_ref().map(|tools| tools.iter().any(|t| t.cache_control.is_some())).unwrap_or(false)
            || m.messages.iter().any(|msg| {
                if let types::MessageContent::Blocks(blocks) = &msg.content {
                    blocks.iter().any(|block| match block {
                        types::RequestContentBlock::Text(tb) => tb.cache_control.is_some(),
                        types::RequestContentBlock::Image(ib) => ib.cache_control.is_some(),
                        types::RequestContentBlock::Document(db) => db.cache_control.is_some(),
                        types::RequestContentBlock::ToolUse(tu) => tu.cache_control.is_some(),
                        types::RequestContentBlock::ToolResult(tr) => tr.cache_control.is_some(),
                        types::RequestContentBlock::SearchResult(sr) => sr.cache_control.is_some(),
                        types::RequestContentBlock::Thinking(_) => false,
                        types::RequestContentBlock::RedactedThinking(_) => false,
                                })
                } else {
            ...
            Some(universal::CachePlan {
                strategy: universal::CacheStrategy::SingleBoundary,
                boundary: universal::BoundaryLocation::EndOfStaticPrefix,
                ttl_hint: Some("5m".to_string()),
            })
        } else {
            None
        };
        ...
        Ok(universal::Request {
            ...
            tool_choice: m.tool_choice.as_ref().map(|tc| convert_tool_choice(tc).0),
            ...
            service_tier: None,
            stream_options: None,
            parallel_tool_calls: m.tool_choice.as_ref().and_then(|tc| convert_tool_choice(tc).1),
            user: None,
    ...
    pub fn encode_universal_to_messages(u: &universal::Response) -> Result<types::MessagesResponse, AIError> {
        ...
        Ok(types::MessagesResponse {
            ...
            usage: types::Usage {
                ...
                // Normalize cache usage from provider-specific fields
                // For now, these will be populated by provider-specific response handling
            ...
            MessageContent::Blocks(blocks) => {
                for block in blocks {
                    match block {
                        ...
                        RequestContentBlock::RedactedThinking(_) => token_count += 50, // Estimate for redacted content
                    }
                }
            }
        }
            ...
            MessageContent::Blocks(blocks) => {
                for block in blocks {
                    match block {
                        ...
                        RequestContentBlock::Thinking(thinking) => {
                            let preview = if thinking.thinking.len() > 50 {
                                format!("{}...", &thinking.thinking[..50])
                            } else {
                                thinking.thinking.clone()
                            };
                            text_parts.push(format!("{}: [Thinking: {}]", role_prefix, preview));
                        },
                        RequestContentBlock::RedactedThinking(_) => {
                            text_parts.push(format!("{}: [Redacted Thinking]", role_prefix));
                        },
                    }
                }
            }
...
pub fn extract_text_from_blocks(blocks: &[RequestContentBlock]) -> String {
    ...
    for block in blocks {
        match block {
            ...
            RequestContentBlock::Thinking(_) => {
                // Don't include thinking blocks in extracted text
            },
            RequestContentBlock::RedactedThinking(_) => {
                // Don't include redacted thinking blocks in extracted text
            },
            _ => {}
        }
    }
...
mod tests {
    ...
    mod header_tests {
        ...
        #[test]
        fn test_extract_anthropic_headers() {
            // Test no headers
            let empty_headers = HeaderMap::new();
            assert!(ingress::extract_anthropic_headers(&empty_headers).is_none());

            // Test version header only
            let mut version_headers = HeaderMap::new();
            version_headers.insert("anthropic-version", HeaderValue::from_static("2023-06-01"));
            let version_data = ingress::extract_anthropic_headers(&version_headers).unwrap();
            assert_eq!(version_data["anthropic"]["headers"]["version"], "2023-06-01");

            // Test version + beta headers
            let mut full_headers = HeaderMap::new();
            full_headers.insert("anthropic-version", HeaderValue::from_static("2023-06-01"));
            full_headers.insert("anthropic-beta", HeaderValue::from_static("feature1,feature2,feature3"));
            let full_data = ingress::extract_anthropic_headers(&full_headers).unwrap();
            let headers_obj = &full_data["anthropic"]["headers"];
