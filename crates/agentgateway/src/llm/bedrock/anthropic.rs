...
impl Provider {
	...
	pub async fn process_request(
		...
		let bedrock_request = translate_request(anthropic_request.clone(), &self.common, anthropic_headers)?;
	...
	pub fn get_path_for_model(&self, model_id: &str, is_streaming: bool) -> String {
		self.common.converse_path(model_id, is_streaming).to_string()
	}
	...
	fn handle_content_block_stop(
		...
		if let Some(metadata) = self.content_block_metadata.get(&index) {
			if metadata.block_type == ContentBlockType::ToolUse {
				if let Some(_json_buffer) = self.tool_json_buffers.remove(&index) {
				...
				match this.decoder.decode(this.decode_buffer) {
					Ok(Some(message)) => {
						...
						if let Ok(bedrock_event) = super::types::ConverseStreamOutput::deserialize(message) {
							match this.processor.process_event(bedrock_event) {
								Ok(anthropic_events) => {
									// Track first token for telemetry
									if !*this.seen_first_token {
										for event in &anthropic_events {
											if matches!(event, StreamEvent::ContentBlockDelta { .. }) {
												*this.seen_first_token = true;
												this.log.non_atomic_mutate(|r| {
...
fn translate_message(
	...
	let content = translate_content_blocks(anthropic_message.content.to_blocks())?;
	...
	let result = match anthropic_block {
		...
		RequestContentBlock::Thinking(thinking_block) => {
			...
			let reasoning_text = bedrock::ReasoningTextBlock {
				...
				signature: Some(thinking_block.signature),
			};
...
fn translate_document_block(
	...
	let (format, name, source) = match anthropic_doc.source {
		...
		anthropic::DocumentSource::UrlPdf { url } => {
			...
			let name = format!("url_placeholder.pdf");
			let placeholder_data = format!("URL_PLACEHOLDER:{}", url);
...
fn translate_request_content_blocks_to_content_blocks(
	...
	blocks
		.iter()
		.map(|block| translate_content_block(block.clone()))
		.collect()
}
