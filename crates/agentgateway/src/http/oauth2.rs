...
impl OAuth2 {
	...
	pub async fn handle_protocol_endpoint(
		...
		debug!(path = req.uri().path(), "handling oauth2 protocol endpoint");
		let runtime = OAuth2CallContext {
			client,
			policy_client,
			oidc,
	...
	pub(crate) fn matches_protocol_endpoint(&self, path: &str) -> Result<bool, ProxyError> {
		Ok(self.protocol_endpoint_kind(path)?.is_some())
	}

	fn extract_original_url(&self, uri: &http::Uri) -> Option<String> {
		let state_str = Self::query_param(uri, "state")?;
		let state_str = Self::unwrap_shared_callback_state(&state_str);
		let state = self
			.handshake_codec
	...
	async fn handle_callback(
		...
		let state_str = self.unwrap_callback_state(&state_str)?;
	...
	pub(crate) fn observes_session_cookie(&self, headers: &http::HeaderMap) -> bool {
		let cookie_name = self.session_cookie_name();
		let mut found = false;
		Self::for_each_request_cookie(headers, |cookie| {
			if found {
			...
			found = cookie
				...
				.is_some_and(|idx| idx <= MAX_SESSION_COOKIE_CHUNK_INDEX);
		});
		found
	}
	...
	fn encode_shared_callback_state(
		inner_state: String,
		attachment_key: &OAuth2AttachmentKey,
	) -> anyhow::Result<String> {
		let encoded = serde_json::to_vec(&SharedCallbackStateEnvelope {
	...
	fn decode_shared_callback_state(encoded: &str) -> Option<SharedCallbackStateEnvelope> {
		let decoded = URL_SAFE_NO_PAD.decode(encoded).ok()?;
		serde_json::from_slice(&decoded).ok()
	}

	fn unwrap_shared_callback_state(state: &str) -> String {
		Self::decode_shared_callback_state(state)
			.map(|envelope| envelope.inner_state)
			.unwrap_or_else(|| state.to_string())
	}

	fn unwrap_callback_state(&self, state: &str) -> Result<String, ProxyError> {
		let Some(envelope) = Self::decode_shared_callback_state(state) else {
			return Ok(state.to_string());
		};
		if envelope.attachment_key != self.attachment_key.to_string() {
			return Err(ProxyError::OAuth2AuthenticationFailure(Error::Handshake(
				"login state was issued for a different oauth2 policy".into(),
			)));
		}
		Ok(envelope.inner_state)
	}

	pub(crate) fn callback_attachment_key_from_uri(uri: &http::Uri) -> Option<String> {
		let state = Self::query_param(uri, "state")?;
		Self::decode_shared_callback_state(&state).map(|envelope| envelope.attachment_key)
	}
	...
	async fn trigger_auth(
		...
		let shared_state = Self::encode_shared_callback_state(encrypted_state, &self.attachment_key)
			.map_err(|e| {
				ProxyError::from(Error::Internal(format!(
					"failed to encode oauth2 callback state: {e}"
				)))
			})?;
		...
		let mut auth_request = auth_url
			.authorize_url(|| CsrfToken::new(shared_state.clone()))
...
struct SharedCallbackStateEnvelope {
...
mod tests {
	...
	fn test_oauth2() -> OAuth2 {
		OAuth2::new(test_config(), test_attachment_key()).unwrap()
	}
	...
	fn oauth2_new_validates_redirect_uri_rules() {
		...
		for case in cases {
			...
			match case.want_err {
				Some(want_err) => {
					let err = OAuth2::new(config, test_attachment_key()).unwrap_err();
					assert!(
				...
				None => {
					assert!(
						OAuth2::new(config, test_attachment_key()).is_ok(),
						"case {:?} should succeed",
						case.name
					);
				},
	...
	fn oidc_session_cookie_key_stays_compatible_when_provider_id_changes() {
		...
		let legacy_oauth2 = OAuth2::new(legacy, test_attachment_key()).unwrap();

		let mut updated = test_config();
		updated.provider_id = "provider-b".to_string();
		let updated_oauth2 = OAuth2::new(updated, test_attachment_key()).unwrap();
	...
	async fn oauth2_enforce_request_uses_resolved_metadata_without_discovery() {
		...
		let oauth2 = OAuth2::new(config, test_attachment_key()).unwrap();
		...
		*req.uri_mut() = "/private/data".parse().unwrap();
		...
		let response = oauth2
			.enforce_request(&client, &policy_client, oidc.tokens(), &mut req)
			.await
			.unwrap();
	...
	async fn oauth2_callback_ignores_id_token_without_validator_in_explicit_mode() {
		...
		*initial_req.uri_mut() = "/private/data".parse().unwrap();
		...
		let initial = oauth2
			.enforce_request(&client, &policy_client, oidc.tokens(), &mut initial_req)
			.await
			.unwrap();
		...
		*callback_req.uri_mut() = format!("/callback?code=auth-code-1&state={state}")
		...
		let callback = oauth2
			.handle_protocol_endpoint(&client, &policy_client, oidc.tokens(), &mut callback_req)
			.await
	...
	async fn oauth2_callback_rejects_state_from_different_attachment() {
		...
		*initial_req.uri_mut() = "/private/data".parse().unwrap();
		...
		let handshake_state = initiating
			.handshake_codec
			.decrypt_handshake_state(&OAuth2::unwrap_shared_callback_state(&state))
	...
	fn inject_auth_sets_upstream_access_token_extension() {
		let oauth2 = OAuth2::new(test_config(), test_attachment_key()).unwrap();
		let session = SessionState {
			access_token: "token".to_string(),
			refresh_token: None,
			claims: None,
	...
	fn large_session_is_chunked_and_round_trips() {
		...
		assert!(
			set_cookie_values
				.iter()
				.any(|v| v.starts_with(&format!("{cookie_name}.1="))),
			"session cookie should be chunked into multiple cookies"
		);

		let cookie_header =
			request_cookie_header_from_set_cookie_values(&set_cookie_values, cookie_name);
	...
	fn logout_clears_all_cookie_clear_slots() {
		...
		assert!(
			set_cookie_values
				.iter()
				.any(|v| v.starts_with(&format!("{cookie_name}="))),
			"logout must clear base session cookie"
		);
		assert!(
			set_cookie_values
				.iter()
				.any(|v| v.starts_with(&format!("{cookie_name}.{}=", COOKIE_CLEAR_SLOTS))),
	...
	fn session_cookie_max_chunk_index_ignores_out_of_range_chunks() {
		...
		headers.insert(
			http::header::COOKIE,
			HeaderValue::from_str(&format!("{cookie_name}=base; {cookie_name}.999=evil")).unwrap(),
		);
	...
	fn session_cookie_max_chunk_index_reads_split_cookie_headers() {
		...
		headers.append(
			http::header::COOKIE,
			HeaderValue::from_str(&format!("{cookie_name}.8=chunk")).unwrap(),
		);
	...
	fn session_cookie_max_chunk_index_prefers_largest_duplicate_across_split_headers() {
		...
		headers.append(
			http::header::COOKIE,
			HeaderValue::from_str(&format!("{cookie_name}.2=chunk")).unwrap(),
		);
		headers.append(
			http::header::COOKIE,
			HeaderValue::from_str(&format!("{cookie_name}.8=chunk")).unwrap(),
	...
	fn logout_clears_observed_chunk_slots() {
		...
		req_headers.insert(
			http::header::COOKIE,
			HeaderValue::from_str(&format!("{cookie_name}=base; {cookie_name}.8=chunk")).unwrap(),
		);
		let policy = oauth2
		...
		assert!(
			set_cookie_values
				.iter()
				.any(|v| v.starts_with(&format!("{cookie_name}.8="))),
