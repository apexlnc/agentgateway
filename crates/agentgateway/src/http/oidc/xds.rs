...
pub struct ResolvedOidcConfig {
	...
	/// Present for confidential clients, omitted for public clients
	/// (`token_endpoint_auth == None`). The controller sources this from the
	/// Secret referenced by `ClientSecret` on the policy; it travels over
	/// xDS and is redacted from logs via `ser_redact`.
	#[serde(
		default,
