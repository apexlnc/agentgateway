...
impl OidcProvider {
	...
	fn validate_endpoint_url(
		...
		match parsed.scheme() {
			"https" => Ok(()),
			"http" if crate::http::is_loopback_host(host) || allow_non_loopback_http => Ok(()),
	...
	fn validate_metadata_endpoints(
		...
		allow_insecure_backchannel: bool,
	) -> Result<(), Error> {
		// Front-channel endpoints stay strict regardless of provider backend configuration.
		...
		Self::validate_endpoint_url(
			...
			"token_endpoint",
			allow_insecure_backchannel,
		)?;
		Self::validate_endpoint_url(&metadata.jwks_uri, "jwks_uri", allow_insecure_backchannel)?;
		if let Some(end_session_endpoint) = metadata.end_session_endpoint.as_deref() {
			Self::validate_endpoint_url(end_session_endpoint, "end_session_endpoint", false)?;
		}
		Ok(())
	}
				...
				let result: Result<Arc<OidcMetadata>, Error> = async {
					...
					let metadata: OidcMetadata =
						tokio::time::timeout(OIDC_HTTP_TIMEOUT, crate::json::from_response_body(resp))
							...
							.map_err(|e| Error::Discovery(e.to_string()))?;
					Self::validate_metadata_endpoints(&metadata, ctx.provider_backend.is_some())?;
