...
fn parse_spiffe_identity(data: &[u8]) -> Option<IstioIdentity> {
	...
	let uri = uri.strip_prefix("spiffe://")?;
	let parts: Vec<&str> = uri.split('/').collect();

	// Expected: [trust_domain, "ns", namespace, "sa", service_account]
	// Must be exactly 5 parts to match ztunnel's parsing (src/identity/manager.rs)
	if parts.len() == 5 && parts[1] == "ns" && parts[3] == "sa" {
		Some(IstioIdentity::new(
			strng::new(parts[0]),
			strng::new(parts[2]),
			strng::new(parts[4]),
		))
